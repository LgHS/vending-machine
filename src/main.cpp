...
void MDB_reset()
{
    ...
    if (validateChecksum(recvData, 2) == 1)
    {
        ...
        digitalWrite(LED_ACTIVE, LOW);
        if (_debug_level)
            dbg_println(F("RST: ACK sent"));
    }
    else
    {
        MDB_Write(sNAK);
        if (_debug_level)
            dbg_println(F("RST: NAK sent"));
    }
}
...
void MDB_setup()
{
    ...
    if (recvData[1].data == CONFIG_DATA)
    {
        ...
        if (1) //validateChecksum(recvData, 7))
        {
            ...
            else
            {
                ...
                if (_debug_level)
                    dbg_println(F("Setup recv, config sent"));
            }
        }
        else
        {
    ...
    else if (recvData[1].data == MINMAX_PRICES)
    {
        ...
        if (validateChecksum(recvData, 7))
        {
            ...
            if (_debug_level)
                dbg_println(F("State Change: Disabled!"));
        }
        else
        {
            // invalid checksum
